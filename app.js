...
// To check your work, open the file in a web browser and inspect the console 
// output, or run the file in your terminal using node app.js.
...
const inventorsBetween = inventors.filter((inventor) => {
...
const name = inventorNames.map((First, Last) => {
    return first + last
});
...
const birthYears = sortedByBirthYear.sort ((a, b) => a - b
)
...
const reformatPeople = firstLast.map((people) => people.split(','));
...
const isAdult19Up = devs.every((year) => {
    return isEveryone19OrOlder > 19;
});
...
const specificId = comments.find((id) => {
...
const findSpecificMessage = comments.findIndex((comment) => {
    return comment.id === '123523';
});
